...
if(is_array($rate_option_arr) && !empty($rate_option_arr)){
	...
	for($i = $c;$i--;$i > 0 ){
		$v = $rate_option_arr[$i];
		$output .='<input '.(!empty($conditional) ? 'data-conditional-name="'.$name.'" data-conditional="'.esc_attr(base64_decode($conditional)).'"': '' ).' name="'.$name.'" value="'.$v->value.'" id="'.sanitize_title($name).'-'.$v->value.'" class="dhvc-form-value" type="radio">' . "\n";
